...
answers = [
           ...
           (1570089600, 3600, 916900, 916, '2019-10-03'),
           ...
           (1570107600, 3600, 676900, 676, '2019-10-03'),
           (1570111200, 3600, 759600, 759, '2019-10-03'),
           (1570114800, 3600, 695900, 695, '2019-10-03'),
           (1570118400, 3600, 853500, 853, '2019-10-03'),
           (1570122000, 3600, 1229500, 1229, '2019-10-03'),
           (1570125600, 3600, 871100, 871, '2019-10-03'),
           (1570129200, 3600, 826900, 826, '2019-10-03'),
           (1570132800, 3600, 1042899, 1042, '2019-10-03'),
           (1570136400, 3600, 1233600, 1233, '2019-10-03'),
           (1570140000, 3600, 1115900, 1115, '2019-10-03'),
           ...
           (1570158000, 3600, 5541900, 5541, '2019-10-03'),
...
class TestHelpers(unittest.TestCase):
    ...
    def test_parse_espi(self):
        ...
        self.assertEqual(dump[0], (1508396400, 3600, 446800, 446, '2017-10-19'))
    ...
    def test_database_daily_write(self):
        ...
        cur.execute("SELECT watt_hours from espi WHERE date='2017-10-19'")
        wh_readings = cur.fetchall()
        wh_readings.sort()
        baseline_ans = int(round(
            (wh_readings[0][0] + wh_readings[1][0] + wh_readings[2][0]) / 3))

        cur.execute("SELECT baseline FROM daily WHERE date='2017-10-19'")
        baseline = cur.fetchall()[0][0]

        cur.execute("SELECT watt_hours from espi WHERE date='2019-10-17'")
        wh_readings = cur.fetchall()
        wh_readings.sort()
        baseline_ans = int(round(
            (wh_readings[0][0] + wh_readings[1][0] + wh_readings[2][0]) / 3))

        cur.execute("SELECT baseline FROM daily WHERE date='2019-10-17'")
        baseline = cur.fetchall()[0][0]
